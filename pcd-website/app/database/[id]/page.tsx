...
export default async function EntryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const pocketLabelList = entry.sequence.pocket_lining_residues.split("-");
  const pdbUrl = entry.assets.pdb_structure ?? "";
  const bindingModes = Object.entries(entry.chaperone.binding_mode);
      ...
      <main className="max-w-7xl mx-auto px-6 pt-20 pb-16 space-y-6">
        ...
        <div className="card rounded-xl p-6 grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="tag tag-violet">{entry.metadata.variant_class}</span>
              <span className="tag tag-green">{entry.status}</span>
            </div>
            <h1 className="text-2xl font-black" style={{ color: "var(--text-primary)" }}>{entry.metadata.disease}</h1>
            <p className="text-sm leading-6" style={{ color: "var(--text-secondary)" }}>{entry.metadata.mechanism}</p>
            <div className="flex flex-wrap gap-4 text-xs font-mono" style={{ color: "var(--text-muted)" }}>
              <span>UniProt <span style={{ color: "var(--text-secondary)" }}>{entry.metadata.uniprot}</span></span>
              <span>ClinVar <span style={{ color: "var(--text-secondary)" }}>#{entry.metadata.clinvar_id}</span></span>
              <span>Precursor <span style={{ color: "var(--text-secondary)" }}>{entry.metadata.mutation_precursor}</span></span>
            </div>
          </div>
          <div className="space-y-4">
            <div className="section-label">Key Scores</div>
            <ScoreRow label="fpocket Druggability" value={entry.pocket.fpocket_druggability} color="var(--accent)" />
            <ScoreRow label="Chaperone Composite" value={entry.chaperone.composite_score} color="var(--violet)" />
            <ScoreRow label="Pocket Affinity" value={entry.chaperone.pocket_affinity_score} color="#3b82f6" />
            <ScoreRow label="Drug-likeness (QED)" value={entry.chaperone.qed} color="var(--green)" />
          </div>
        </div>
        ...
        {pdbUrl && (
          <div className="space-y-3">
            ...
            <p className="text-xs" style={{ color: "var(--text-muted)" }}>
              {entry.pocket.target_conformation}
            </p>
          </div>
        )}
        ...
        <div className="grid lg:grid-cols-2 gap-5">
          ...
          <div className="card rounded-xl p-5 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                { l: "fpocket Drug.", v: entry.pocket.fpocket_druggability.toFixed(3), c: "var(--accent)" },
                { l: "WT Baseline", v: entry.pocket.wt_baseline_druggability.toFixed(3), c: "var(--red)" },
                { l: "Volume (Å³)", v: entry.pocket.volume_angstrom3.toFixed(1), c: "var(--text-secondary)" },
                { l: "α-Spheres", v: String(entry.pocket.alpha_sphere_count), c: "var(--text-secondary)" },
                { l: "Conformations", v: String(entry.pocket.n_conformations_sampled), c: "var(--text-secondary)" },
                { l: "Mut→Pocket Dist.", v: `${entry.pocket.dist_mutation_to_pocket_angstrom.toFixed(1)} Å`, c: "var(--amber)" },
              ].map(p => (
            ...
            <div className="rounded-lg border border-[var(--border)] p-3" style={{ background: "var(--bg-1)" }}>
              ...
              <div className="font-mono text-sm" style={{ color: "var(--accent)" }}>
                ({entry.pocket.center_angstrom.map(v => v.toFixed(2)).join(", ")})
              </div>
            </div>

            <div className="rounded-lg border border-[var(--border)] p-3" style={{ background: "var(--bg-1)" }}>
              <div className="data-label mb-1">Pocket Type</div>
              <div className="text-xs" style={{ color: "var(--text-secondary)" }}>{entry.pocket.pocket_type}</div>
            </div>
            ...
            <div className="rounded-lg border border-[var(--border)] p-3" style={{ background: "var(--bg-1)" }}>
              ...
              <div className="font-mono text-[11px] break-all leading-5" style={{ color: "var(--text-secondary)" }}>
                {entry.sequence.sequence_slice_around_pocket}
              </div>
            </div>
          </div>
          ...
          <div className="card rounded-xl p-5 space-y-5">
            <div>
              ...
              <h2 className="mt-3 font-bold" style={{ color: "var(--text-primary)" }}>{entry.chaperone.common_name}</h2>
              <p className="text-xs italic mt-1" style={{ color: "var(--text-secondary)" }}>{entry.chaperone.iupac_name}</p>
            </div>

            <div className="rounded-lg border border-[var(--border)] p-3" style={{ background: "var(--bg-1)" }}>
              <div className="data-label mb-2">SMILES</div>
              <div className="font-mono text-xs break-all" style={{ color: "var(--accent)" }}>
                {entry.chaperone.smiles}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-4 gap-1.5">
                <PropCell label="MW (Da)" value={entry.chaperone.mw.toFixed(0)} />
                <PropCell label="logP" value={entry.chaperone.logp.toFixed(2)} pass={entry.chaperone.logp >= 0 && entry.chaperone.logp <= 5} />
                <PropCell label="QED" value={entry.chaperone.qed.toFixed(3)} pass={entry.chaperone.qed > 0.6} />
                <PropCell label="SA" value={entry.chaperone.sa_score.toFixed(1)} pass={entry.chaperone.sa_score < 4} />
                <PropCell label="HBD" value={String(entry.chaperone.hbd)} pass={entry.chaperone.hbd <= 5} />
                <PropCell label="HBA" value={String(entry.chaperone.hba)} pass={entry.chaperone.hba <= 10} />
                <PropCell label="TPSA" value={entry.chaperone.tpsa.toFixed(0)} pass={entry.chaperone.tpsa <= 140} />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  { label: "Lipinski Ro5", pass: entry.chaperone.lipinski },
                  { label: "Veber Oral", pass: entry.chaperone.veber },
                  { label: "PAINS-free", pass: !entry.chaperone.pains },
                ].map(r => (
        ...
        <div className="card rounded-xl p-5 space-y-4">
          ...
          <div className="flex gap-4 text-xs font-mono" style={{ color: "var(--text-muted)" }}>
            <span>Shape <span style={{ color: "var(--text-secondary)" }}>{entry.eij_matrix.shape[0]} × {entry.eij_matrix.shape[1]}</span></span>
            <span>Source <span style={{ color: "var(--text-secondary)" }}>{entry.eij_matrix.file}</span></span>
          </div>
          <EijHeatmap labels={entry.eij_matrix.residue_labels} values={entry.eij_matrix.values ?? []} />
        </div>
        ...
        <div className="card rounded-xl p-5 space-y-4">
          <div>
            ...
            <p className="mt-3 text-xs font-mono" style={{ color: "var(--text-muted)" }}>{entry.sequence.fasta_header}</p>
          </div>
          <div className="rounded-lg border border-[var(--border)] p-4 overflow-x-auto"
            ...
            <div className="font-mono text-[11px] break-all leading-5">
              {entry.sequence.fasta_sequence.split("").map((ch, i) => {
                const pos = i + 1;
                const isLining = entry.sequence.pocket_lining_positions_precursor?.includes(pos);
                return (
                  <span key={i}
          ...
          <div className="text-xs font-mono" style={{ color: "var(--text-muted)" }}>
            1-letter: <span className="font-bold tracking-widest" style={{ color: "var(--text-secondary)" }}>
              {entry.sequence.pocket_lining_1letter}
            </span>
          </div>
        </div>
        ...
        <div className="card rounded-xl p-5 space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-2">
            {[
              { label: "E_ij Matrix (.npy)", path: entry.assets.eij_matrix_npy },
              ...(entry.assets.eij_matrix_csv ? [{ label: "E_ij Matrix (.csv)", path: entry.assets.eij_matrix_csv }] : []),
              { label: "Transient Conformation (.pdb)", path: entry.assets.transient_conformation_pdb },
              { label: "Pocket Summary (.json)", path: entry.assets.pocket_summary_json },
              { label: "Stage 3 Candidates (.json)", path: entry.assets.stage3_candidates_json },
            ].map(a => (
